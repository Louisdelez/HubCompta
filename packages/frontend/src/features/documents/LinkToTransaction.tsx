    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-ctp-base rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-hidden animate-scale-in">
        <div className="p-6 border-b border-ctp-surface1">
          <h2 className="text-xl font-bold">{t('documents.linkToTransaction.title')}</h2>
          <p className="text-sm text-ctp-subtext0 mt-1">
            {t('documents.linkToTransaction.description', { filename: document.filename })}
          </p>
        </div>
        ...
        <div className="p-4 border-b border-ctp-surface1">
          <input
            type="text"
            placeholder={t('documents.linkToTransaction.searchTransaction')}
            value={searchQuery}
        ...
        <div className="max-h-96 overflow-y-auto">
          {isLoading ? (
            <div className="p-6 text-center">
              ...
              <p className="text-ctp-subtext0">{t('documents.linkToTransaction.loading')}</p>
            </div>
            ...
            <div className="p-6 text-center">
              <p className="text-ctp-subtext0">
                {searchQuery ? t('documents.linkToTransaction.noResults') : t('documents.linkToTransaction.noRecentTransactions')}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-ctp-surface1 flex gap-3">
          <button onClick={onClose} className="btn-secondary flex-1">
            {t('documents.linkToTransaction.cancel')}
          </button>
          <button
            onClick={handleLink}
          ...
          >
            {linkMutation.isPending ? t('documents.linkToTransaction.linking') : t('documents.linkToTransaction.link')}
          </button>
        </div>
      </div>
    </div>
